# =============================================================================
# Engine v1.5.10 — EXPERIMENTAL — direction-aware basket evaluator (extends the v1.5.9 extraction of v1.5.8's per-bar block).
#
# Purpose:
...
# =============================================================================
"""
Universal Research Engine v1.5.10 — Per-bar Evaluator (EXPERIMENTAL).
...
def _bar_spread(row: pd.Series) -> float:
    ...
    0.0 when absent / NaN / <=0 -> direction-aware fills are a no-op (byte-identical
    to frozen v1.5.9 on spread=0 data)."""
...
def _exec_fill(raw_ask_price: float, is_sell: bool, bar_spread: float) -> float:
    """Direction-aware execution price (v1.5.10). BUY -> ask (raw); SELL -> bid
    (= ask - embedded spread). Restores the OctaFx addendum ASK-for-BUY /
    BID-for-SELL intent. P&L formula unchanged — only the fill price becomes
    side-correct."""
...
def evaluate_bar(
    ...
    if not state.in_pos:
        ...
        if exit_triggered:
            # v1.5.10: long exit=SELL (bid); short exit=BUY (ask).
            exit_price = _exec_fill(
                exit_price, is_sell=(state.direction == 1), bar_spread=_bar_spread(row)
            )
            ...
            if guards_pass:
                ...
                if partial_sig is not None:
                    ...
                    state.partial_leg = {
                        ...
                        'exit_price':     _exec_fill(
                            row['close'], is_sell=(state.direction == 1),
                            bar_spread=_bar_spread(row)),
...
def finalize_force_close(df: pd.DataFrame, state: BarState, trades: list[dict[str, Any]]) -> None:
    ...
    if state.in_pos and len(df) > 0:
        ...
        trade = {
            ...
            "exit_price":         _exec_fill(
                                      last_row['close'], is_sell=(state.direction == 1),
