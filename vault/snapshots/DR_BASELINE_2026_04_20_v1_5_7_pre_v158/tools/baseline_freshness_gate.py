...
def format_blocked_message(result: FreshnessResult) -> str:
    ...
    lines += [
        ...
        f"    1. python tools/reset_directive.py {result.strategy_id} --reason \"refresh stale baseline\"",
        f"    2. Remove any existing rows for {result.strategy_id} from:",
        f"         - strategies/Strategy_Master_Filter.xlsx  (cardinality gate refuses duplicates)",
        f"         - strategies/Master_Portfolio_Sheet.xlsx  (portfolio ledger refuses overwrite)",
        f"    3. python tools/run_pipeline.py {result.strategy_id}",
        f"       (Stages 3-4 re-append Master Filter + MPS with fresh metrics)",
        "",
    ]
    return "\n".join(lines)
