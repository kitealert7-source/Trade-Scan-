...
## 13. Post-RUN_COMPLETE Orchestration Steps

The following steps execute AFTER the core FSM reaches COMPLETE but BEFORE
a directive is considered fully processed. They are workflow-level
orchestration steps governed by `execute-directives.md`, not FSM stages.

These steps do NOT modify Stage-1 through Stage-3A artifacts.
...
- Tool: `tools/profile_selector.py`
- Selects best-performing profile per strategy by Return/DD ratio
- Writes `deployed_profile` to `Master_Portfolio_Sheet.xlsx`
- Governed by SOP_PORTFOLIO_ANALYSIS §4.6

### 13.3 Workspace Reconciliation (Step 12 in workflow)

- Tool: `tools/cleanup_reconciler.py`
- Dry-run → Execute → Convergence check (0 actions remaining)
- Governed by SOP_CLEANUP §8.5

### 13.4 Artifact Formatting (Step 13 in workflow)

- Tool: `tools/format_excel_artifact.py`
- Applies unified formatting to all Excel ledgers
- MUST run after reconciliation to format the final state

These steps are idempotent and deterministic.
Failure in any post-RUN_COMPLETE step does NOT invalidate the run itself.

---

**End of SOP_TESTING — VERSION 2.2**
