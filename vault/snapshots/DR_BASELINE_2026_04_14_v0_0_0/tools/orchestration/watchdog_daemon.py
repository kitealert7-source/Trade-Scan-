...
def run_watchdog_loop() -> None:
    ...
    while True:
        try:
            ...
            # (e.g. CONFIG_INTEGRITY_FAIL exit code 78). Restart immediately
            # instead of waiting for heartbeat stall (saves up to 300s blind time).
            if EXEC_PID.exists():
                ...
                if _exit_pid is not None and not _pid_is_alive(_exit_pid):
                    ...
                    if _uptime < EARLY_EXIT_GRACE_S and hb_age is None:
                        ...
                        time.sleep(POLL_INTERVAL_S)
                        continue
                    _log(f"EARLY_EXIT_DETECTED | pid={_exit_pid} | process died with PID file intact — immediate restart")
                    _send_alert("EARLY_EXIT_DETECTED",
                        f"pid={_exit_pid} died abnormally (PID file still exists). Restarting immediately.")
