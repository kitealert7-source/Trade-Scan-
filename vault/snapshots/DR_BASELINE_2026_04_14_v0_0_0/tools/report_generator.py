...
def generate_backtest_report(directive_name: str, backtest_root: Path, *,
    ...
    if fill_age_data:
        md.append("## Regime Lifecycle (Fill Age \u2014 Execution State)\n")
        md.append("*Execution-state view of regime lifecycle (measured at fill bar, not signal).*\n")
    ...
    if delta_age_data:
        md.append("## Age Delta (Fill \u2212 Signal) Distribution\n")
        md.append("*Delta = fill_age \u2212 signal_age (positive = aging, negative = reset/transition).*\n")
