...
def compute_age_dual_breakdown(trades: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Single pass over trades. Produces three parallel views:
      - signal_buckets: regime_age_signal (state at decision time)
      - fill_buckets:   regime_age_fill   (state at actual fill under next_bar_open)
      - delta_buckets:  fill_age - signal_age (positive = aging, negative = reset/transition)
