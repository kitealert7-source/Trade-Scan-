...
def run_preflight(
    ...
    # Hash-based: the approval marker stores sha256(strategy.py). Approval is
    # valid when the current file's hash equals the stored hash. mtime is
    # irrelevant — byte-identical rewrites (provisioner, --rehash) cannot trip
    # the gate. All legacy markers were bulk-upgraded to hash-based on 2026-04-18.
    if _strategy_name:
        ...
        if _is_new_strategy:
            ...
            if not is_approval_current(_strat_py, _approved):
                return (
                    "AWAITING_HUMAN_APPROVAL",
                    f"EXPERIMENT_DISCIPLINE: strategy.py content hash differs from last approval "
                    f"(or marker missing). Do NOT reset and re-run this directive. "
                    f"Create a new directive version (e.g. {_next_ver}) and run fresh. "
                    f"If this is intentional, delete 'strategy.py.approved' in "
                    f"strategies/{_strategy_name}/ and re-run — the Auto-Consistency Gate "
                    f"will regenerate the marker with the new hash.",
                    None,
                )
