...
def update_master_portfolio_ledger(strategy_id, metrics, corr_data, max_stress_corr, concurrency_data, constituent_run_ids, n_assets=1, allow_supersede=False):
    """
    ...
    if strategy_id in df_ledger["portfolio_id"].astype(str).values:
        ...
        if allow_supersede:
            print(
                f"  [LEDGER] Portfolio '{strategy_id}' exists with different metrics; "
                f"superseding old entry (--allow-supersede authorized)."
            )
            df_ledger = df_ledger[df_ledger["portfolio_id"].astype(str) != strategy_id].copy()
        else:
            raise ValueError(
...
def main():
    ...
    parser.add_argument("--allow-supersede", action="store_true", help="Allow overwriting an existing portfolio ledger entry (ENGINE reruns only).")
    ...
    if is_valid_for_master:
        ...
        try:
            update_master_portfolio_ledger(strategy_id, port_metrics, corr_data, max_stress_corr, concurrency_data, unique_runs, n_assets=len(symbol_trades), allow_supersede=getattr(args, 'allow_supersede', False))
