...
# Leaf paths whose value-change is a directional-behavior edit (SIGNAL-level),
# not a structural violation. Whitelist is strict: only named paths are
# promoted out of `structural` — any other shape change still fails closed.
# Field removal/addition is NOT covered here (that remains structural).
_DIRECTION_SIGNAL_PATHS = frozenset({
    "trade_management.direction_restriction",
    "state_machine.entry.direction",
    "state_machine.exit.direction",
})

# Portfolio/execution infrastructure knobs that sit OUTSIDE the research signal.
# These do not change the entry/exit logic the strategy computes; they govern
# post-Stage-1 capital / dispatch behavior shared across research, burn-in,
# and live. Adding/removing such a key is treated as a PARAMETER (numeric)
# delta, not a structural violation, so directives that activate the knob
# for the first time still classify mechanically.
# Whitelist is strict: only named paths qualify.
_INFRA_KNOB_NUMERIC_PATHS = frozenset({
    "execution.concurrency_cap",
})
...
def _partition_diffs(
    ...
    for key in all_keys:
        ...
        if top in _COSMETIC_KEYS:
            ...
            identity.add(key)
        elif key in _INFRA_KNOB_NUMERIC_PATHS:
            # Whitelisted infra knob: field addition/removal counts as
            # PARAMETER if the present side is numeric (or None -> numeric).
            present = vb if va is _MISSING else va
            other = va if va is _MISSING else vb
            if va is _MISSING or vb is _MISSING:
                if _is_numeric(present) or present is None:
                    numeric.add(key)
                else:
                    structural.add(key)
            elif _is_numeric(va) and _is_numeric(vb):
                numeric.add(key)
            elif (va is None and _is_numeric(vb)) or (vb is None and _is_numeric(va)):
                numeric.add(key)
            else:
                structural.add(key)
...
def classify_diff(
    ...
    *,
    ...
    # Rule 1b: directional-field value change (whitelisted paths only) -> SIGNAL.
    # Only fires when the leaf exists on BOTH sides (a value change, not a
    # field addition/removal — those stay structural to preserve fail-closed).
    direction_diffs = sorted(
        p for p in (structural | numeric) & _DIRECTION_SIGNAL_PATHS
        if p in leaves_a and p in leaves_b
    )
    if direction_diffs:
        remaining_structural = structural - set(direction_diffs)
        if not remaining_structural:
            return {
                "classification": "SIGNAL",
                "reason": f"direction-field value change: {direction_diffs}",
                "indicator_import_delta": {"removed": [], "added": []},
                "indicator_hash_delta": [],
                "numeric_diffs": sorted(numeric - set(direction_diffs)),
                "cosmetic_diffs": sorted(cosmetic),
                "structural_diffs": [],
            }
        # Direction change PLUS other structural diffs -> still fail closed.
