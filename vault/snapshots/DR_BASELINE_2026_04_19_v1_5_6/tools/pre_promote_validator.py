...
from tools.pipeline_utils import parse_directive, resolve_engine_version_from_directive
...
def _locate_directive_for_strategy(strategy_id: str) -> Path | None:
    """Find the directive .txt for a strategy_id.

    Strategies may carry a trailing _<SYMBOL> suffix not present in the
    directive stem (e.g. 02_..._P01_AUS200 → directive 02_..._P01.txt).
    Try exact match first, then progressively strip trailing tokens.
    """
    search_dirs = ("completed", "active", "active_backup", "INBOX", "INBOX_QUEUED")
    parts = strategy_id.split("_")
    # Walk from most-specific to least-specific stem.
    for stop in range(len(parts), 0, -1):
        stem = "_".join(parts[:stop])
        for sub in search_dirs:
            cand = DIRECTIVES_ROOT / sub / f"{stem}.txt"
            if cand.exists():
                return cand
    return None


def _check_experimental_engine(strategy_id: str, result: LayerResult) -> None:
    """BLOCK promotion when the strategy's directive targets an EXPERIMENTAL engine.

    Reads engine_version via resolve_engine_version_from_directive(), loads the
    matching engine_manifest.json, and fails if engine_status == 'EXPERIMENTAL'.
    Missing directive → WARN (legacy strategies without a locatable directive
    default to the canonical FROZEN engine via the resolver, which is safe).
    """
    directive_path = _locate_directive_for_strategy(strategy_id)
    if directive_path is None:
        result.add("engine_not_experimental", "WARN",
                   f"No directive found for {strategy_id} — cannot verify engine status")
        return

    try:
        directive = parse_directive(directive_path)
    except Exception as e:
        result.add("engine_not_experimental", "FAIL",
                   f"Directive parse failed ({directive_path.name}): {e}")
        return

    ver = resolve_engine_version_from_directive(directive)
    ver_underscore = "v" + ver.replace(".", "_")
    manifest_path = ENGINE_DEV_ROOT / ver_underscore / "engine_manifest.json"

    if not manifest_path.exists():
        result.add("engine_not_experimental", "FAIL",
                   f"Engine manifest not found: {manifest_path.relative_to(PROJECT_ROOT)}")
        return
    ...
    try:
        ...
        result.add("engine_not_experimental", "FAIL",
                   f"Manifest parse failed ({manifest_path.name}): {e}")
        return

    status = str(manifest.get("engine_status", "")).upper()
    if status == "EXPERIMENTAL":
        result.add("engine_not_experimental", "FAIL",
                   f"Engine {ver_underscore} is EXPERIMENTAL — promotion blocked. "
                   f"Wait for freeze (engine_status=FROZEN, vaulted=true) before promoting.")
    elif status == "FROZEN":
        result.add("engine_not_experimental", "PASS", f"engine={ver_underscore} FROZEN")
    else:
        result.add("engine_not_experimental", "FAIL",
                   f"Engine {ver_underscore} has unexpected status='{status}' — expected FROZEN")
...
def validate_layer1(strategy_id: str, result: LayerResult) -> None:
    ...
    _check_experimental_engine(strategy_id, result)
