...
def _load_superseded_stems() -> set[str]:
    """Return stems listed as superseded in governance/supersession_map.yaml.

    Priors whose stem appears here must not contribute to (model, asset_class)
    match pools or prior-max signal_version computation — the canonical
    successor represents that lineage. Cached on first call.
    """
    cache = getattr(_load_superseded_stems, "_cache", None)
    if cache is not None:
        return cache
    try:
        import yaml
        with open(_SUPERSESSION_MAP_PATH, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f) or {}
        supers = data.get("supersessions") or {}
        result = {str(k) for k in supers.keys()} if isinstance(supers, dict) else set()
    except (OSError, ImportError, Exception):
        result = set()
    _load_superseded_stems._cache = result  # type: ignore[attr-defined]
    return result
...
def _extract_timeframe(directive: dict) -> str:
    """Return uppercase TF token parsed from directive strategy name, or ''."""
    strategy_name = str(directive.get("strategy") or directive.get("name") or "")
    ...
    if not parsed:
        return ""
    return str(parsed.get("timeframe", "")).upper()
...
def _find_prior_matches(
    ...
    *,
    ...
    """Return prior directives matching (model, asset_class[, timeframe]),
    each as (path, parsed_dict, signal_version). Excludes the current
    directive by path. Sorted by mtime descending (most recent first).
    ...
    current_timeframe: when non-empty, priors whose parsed timeframe differs
    are excluded. Monotonic signal_version across timeframes is invalid —
    a 1D sv=9 must not gate a 4H sv=2 admission. Empty string falls back to
    legacy (model, asset_class)-only matching for directives whose strategy
    name can't be parsed for a timeframe token.
    """
    current_resolved = current_path.resolve()
    current_stem = current_path.stem
    superseded = _load_superseded_stems()
    tf_filter = current_timeframe.upper().strip()
    matches: list[tuple[Path, dict, int, float]] = []
    for path in _iter_prior_directives(search_dirs):
        if path.resolve() == current_resolved:
            continue
        # Superseded directives are represented by their canonical successor.
        ...
        if m == current_model and ac == current_asset_class:
            if tf_filter:
                prior_tf = _extract_timeframe(parsed)
                if prior_tf and prior_tf != tf_filter:
                    continue
            try:
...
def _find_prior_matches_narrowed(
    ...
    cur_tf = _extract_timeframe(current_directive)
