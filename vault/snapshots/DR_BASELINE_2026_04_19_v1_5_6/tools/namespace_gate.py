...
# Append-only drift metric: each namespace admission emits one JSONL row.
# Tally the file to compute the slot3_symbol_usage rate over time; if the
# ratio doesn't trend to zero, the system is still generating bad directives
# and the advisory needs to be promoted to a hard block.
SLOT3_USAGE_LOG = PROJECT_ROOT / "outputs" / "logs" / "slot3_usage.jsonl"
...
def _record_slot3_usage(strategy_name: str, slot3_kind: str, layer_violation: bool) -> None:
    """Append one JSONL row to the drift-metric sidecar. Best-effort — never
    raises. Each admission writes: {ts, directive, slot3_kind, layer_violation}.
    Tally example: `jq -s 'group_by(.slot3_kind) | map({k: .[0].slot3_kind, n: length})'`.
    """
    try:
        SLOT3_USAGE_LOG.parent.mkdir(parents=True, exist_ok=True)
        row = {
            "ts": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "directive": strategy_name,
            "slot3_kind": slot3_kind,
            "layer_violation": bool(layer_violation),
        }
        with SLOT3_USAGE_LOG.open("a", encoding="utf-8") as f:
            f.write(json.dumps(row) + "\n")
    except Exception as _e:  # defensive — metric must never break admission
        print(f"[NAMESPACE_GATE] WARNING: slot3 metric write failed: {_e}",
              file=sys.stderr)
...
def validate_namespace(directive_path: str | Path) -> dict[str, str]:
    ...
    allowed_asset_classes = {_norm_token(v) for v in token_dict.get("asset_class", [])}
    allowed_symbols = {_norm_token(v) for v in token_dict.get("symbol", [])}
    ...
    # SLOT-3 validation: must be either a known asset_class or a known symbol.
    # Both whitelists are seeded from token_dictionary.yaml. A token that
    # matches neither is almost always a typo (or a new symbol that needs
    # registering before the directive can admit).
    slot3 = _norm_token(m.group("symbol"))
    if allowed_asset_classes or allowed_symbols:
        if slot3 in allowed_asset_classes:
            slot3_kind = "asset_class"
        elif slot3 in allowed_symbols:
            slot3_kind = "symbol"
        else:
            raise NamespaceValidationError(
                f"NAMESPACE_TOKEN_INVALID: SLOT-3='{slot3}' is neither an "
                f"asset_class nor a symbol in token_dictionary.yaml. "
                f"Allowed asset_class: {sorted(allowed_asset_classes)}. "
                f"Register the symbol in token_dictionary.symbol if this is "
                f"a new pair, or correct the typo."
            )
    else:
        # Graceful fallback: if token_dictionary hasn't shipped the whitelists
        # yet (transitional state), keep legacy behavior (accept any token).
        slot3_kind = "unknown"
    symbol = slot3  # preserve legacy field name for downstream consumers
    ...
    # Layer-violation advisory: the directive's SLOT-3 is a specific symbol
    # but the idea is registered at class scope (instruments: FX / IDX / XAU /
    # BTC / ETH / CRYPTO). The stem therefore hardcodes a single symbol into
    # what the idea_registry declares as a broad-scope probe — Execution layer
    # bleeding into the Directive layer. Advisory only; does NOT block admission
    # (existing families rely on this for legacy reasons — see migration plan).
    #
    # End-state direction: SLOT-3 ∈ asset_class ONLY. Symbols belong to the
    # execution layer (per-symbol folders), not the directive layer. This
    # advisory exists to drive that migration — see the LAYER_VIOLATION metric
    # sidecar emitted below; if the rate doesn't trend to zero over time,
    # promote this advisory to a hard block.
    reg_instruments = _norm_token(idea_rec.get("instruments", ""))
    layer_violation = (
        slot3_kind == "symbol"
        and reg_instruments in allowed_asset_classes
        and reg_instruments != ""
    )
    if layer_violation:
        # Structured, grep-able single-line payload — future agents/scripts can
        # tally with `grep '\[ADVISORY\]\[LAYER_VIOLATION\]'` over logs. Keys are
        # k=v pairs so the record survives ad-hoc log munging.
        print(
            f"[ADVISORY][LAYER_VIOLATION] "
            f"directive={strategy_name} "
            f"expected=ASSET_CLASS({reg_instruments}) "
            f"received=SYMBOL({slot3}) "
            f"suggested_stem={idea_id}_{family}_{reg_instruments}_{timeframe}_{model} "
            f"action=migration_recommended",
            file=sys.stderr,
        )
    _record_slot3_usage(strategy_name, slot3_kind, layer_violation)
