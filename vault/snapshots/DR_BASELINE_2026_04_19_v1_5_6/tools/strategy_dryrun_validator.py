...
def _write_dryrun_stats(directive_id: str, *, signal_count: int, sample_bars: int,
                        sample_symbol: str, executed: bool) -> None:
    """Persist a small sidecar so Stage-1 can detect silent-zero regressions.

    Layout: TradeScan_State/runs/<directive_id>/dryrun_stats.json. The file is
    a stable contract consumed by run_stage1_execution() to decide whether a
    0-trade backtest deserves SILENT_ZERO_TRADES escalation.
    """
    try:
        out_dir = RUNS_DIR / directive_id
        ...
        (out_dir / "dryrun_stats.json").write_text(
            json.dumps(payload, indent=2), encoding="utf-8",
        )
    except Exception as _e:
        print(f"[DRYRUN] WARNING: Failed to persist dryrun_stats.json: {_e}")
...
def validate_strategy_dryrun(directive_id: str, first_symbol: str, directive_path) -> bool:
    ...
    if not files:
        ...
        _write_dryrun_stats(directive_id, signal_count=0, sample_bars=0,
                            sample_symbol=first_symbol, executed=False)
        return True
    ...
    # Persist the stats sidecar — Stage-1 reads this to detect silent-zero regressions.
    _write_dryrun_stats(directive_id, signal_count=signal_count, sample_bars=len(df),
                        sample_symbol=first_symbol, executed=True)
