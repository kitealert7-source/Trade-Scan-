...
def run_portfolio_and_post_stages(
    *,
    ...
    if is_multi_asset:
        ...
        # ENGINE reruns (same strategy, new run_ids) are authorized to supersede the
        # existing portfolio ledger entry. Detected via the structured override prefix
        # written by tools/rerun_backtest.py prepare --category ENGINE.
        _override_reason = str((p_conf.get("test") or {}).get("repeat_override_reason") or "")
        if _override_reason.upper().startswith("[RERUN:ENGINE@"):
            cmd_args.append("--allow-supersede")
            print(f"[STAGE-4] ENGINE rerun detected — passing --allow-supersede to portfolio_evaluator.")
