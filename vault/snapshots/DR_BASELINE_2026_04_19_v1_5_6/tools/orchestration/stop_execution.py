...
# Canonical stop script lives in TS_Execution. Renamed from stop_execution.py
# to stop_burnin.py — keep this path in sync or the Friday shutdown task will
# silently no-op with exit code 2 (python: can't open file).
_CANONICAL_STOP = TS_EXEC_ROOT / "tools" / "stop_burnin.py"

LOGS_DIR = TS_EXEC_ROOT / "outputs" / "logs"
STOP_LOG = LOGS_DIR / "stop_execution.log"

# Symbols that trade 24/7 and should NOT be shut down on Fri 22:00 UTC.
# Matched as a substring of the portfolio.yaml symbol field (case-insensitive)
# so BTCUSD, ETHUSD, XBTUSD, SOLUSD, etc. all qualify.
_CRYPTO_TOKENS = ("BTC", "ETH", "XBT", "SOL", "XRP", "DOGE", "LTC", "BCH", "ADA")
PORTFOLIO_YAML = TS_EXEC_ROOT / "portfolio.yaml"


def _has_crypto_in_portfolio() -> tuple[bool, list[str]]:
    """Return (has_crypto, matched_symbols). Safe — never raises."""
    try:
        text = PORTFOLIO_YAML.read_text(encoding="utf-8")
    except Exception as e:
        _log(f"PORTFOLIO_READ_ERROR | {type(e).__name__}: {e} | falling through to normal shutdown")
        return (False, [])
    matched: list[str] = []
    for line in text.splitlines():
        s = line.strip()
        if not s.lower().startswith("symbol:"):
            continue
        # "symbol: BTCUSD" → "BTCUSD"
        val = s.split(":", 1)[1].strip().strip('"').strip("'").upper()
        if any(tok in val for tok in _CRYPTO_TOKENS):
...
def main() -> int:
    ...
    # Crypto keep-alive guard: if portfolio.yaml contains any 24/7 symbol,
    # skip the Friday shutdown entirely. FX/metals/indices threads stay loaded
    # but silent (no bars → no callbacks → no alerts). Watchdog sees BTCUSD's
    # hourly bars so BAR_STALL never trips. Monday resumes naturally.
    # --force flag overrides the guard for manual/emergency shutdowns.
    force = "--force" in sys.argv
    has_crypto, crypto_syms = _has_crypto_in_portfolio()
    if has_crypto and not force:
        _log(f"CRYPTO_KEEPALIVE_SKIP | portfolio contains 24/7 symbols: {','.join(crypto_syms)} "
             f"| skipping shutdown so crypto continues trading through the weekend "
             f"| use --force to override")
        _telegram(f"[WEEKEND_SKIP] Fri shutdown suppressed — {','.join(crypto_syms)} keeps trading 24/7. "
                  f"Daemon stays up; FX threads go silent until Mon 00:00 UTC.")
        return 0
    ...
    _log("STOP_COMPLETE | delegated to TS_Execution/tools/stop_burnin.py")
