...
def get_engine_version(engine_path=None):
    ...
    Env override (per-invocation opt-in): ENGINE_VERSION_OVERRIDE=v1_5_7
    selects engine_dev/universal_research_engine/<value>/main.py. Used for
    EXPERIMENTAL engine probes (e.g. v1.5.7 partial-exit runs) without
    flipping the global pin. Ignored when engine_path is passed explicitly.
    """
    if not engine_path:
        override = os.environ.get("ENGINE_VERSION_OVERRIDE", "").strip()
        if override:
            engine_path = PROJECT_ROOT / f"engine_dev/universal_research_engine/{override}/main.py"
        else:
...
def resolve_engine_version_from_directive(directive: dict, default: str = "1.5.6") -> str:
    ...
    to v1.5.6, which is FROZEN and the current canonical engine).

    This is the single source of truth for engine selection in the pipeline.
    Any tool that needs to pick an engine for a given directive must call this —
    never hardcode a version, never infer from file paths.
    """
