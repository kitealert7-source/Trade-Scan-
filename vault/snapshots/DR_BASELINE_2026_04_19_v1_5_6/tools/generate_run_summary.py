...
def generate(quiet: bool = False) -> Path:
    ...
    # --- Exclude superseded runs (is_current=0 in master_filter) ---
    # Downstream consumers (idea_evaluation_gate) treat run_summary as the
    # live-state view. Rows for runs whose ledger entry was flipped to
    # is_current=0 by a supersession must not appear here.
    try:
        from tools.ledger_db import query_master_filter
        live_ids = set(query_master_filter(include_superseded=False)["run_id"].astype(str).tolist())
        all_ids = set(query_master_filter(include_superseded=True)["run_id"].astype(str).tolist())
        superseded_ids = all_ids - live_ids
        if superseded_ids and "run_id" in df.columns:
            before = len(df)
            df = df[~df["run_id"].astype(str).isin(superseded_ids)]
            if not quiet:
                print(f"  Dropped {before - len(df)} superseded run(s) (is_current=0)")
    except Exception as _exc:  # pragma: no cover - defensive
        if not quiet:
            print(f"  [warn] supersession filter skipped: {_exc}")
