...
# Multi-engine hash verification. FROZEN engines are strictly enforced;
# EXPERIMENTAL ones are verified when a manifest exists but their failure
# does not block the runtime engine's integrity check. Extending this tuple
# when promoting a new successor engine keeps hash drift caught early.
KNOWN_ENGINES = ("1.5.6", "1.5.7")
...
def _verify_manifest(engine_root: Path, manifest_path: Path, *, strict: bool) -> bool:
    """Verify one engine's files against its manifest. Returns True iff all
    file hashes match. `strict=True` means a missing manifest is a failure;
    `strict=False` skips silently (EXPERIMENTAL engines not yet vaulted)."""
    if not manifest_path.exists():
        if strict:
            print(f"[FAIL] Manifest not found at {manifest_path}")
            return False
        print(f"[SKIP] Manifest absent at {manifest_path} (non-strict)")
        return True
    ...
    if not file_hashes:
        print(f"[FAIL] No file_hashes in manifest {manifest_path}")
        return False
    ...
    for filename, expected_hash in file_hashes.items():
        ...
        if actual_hash != expected_hash.upper():
            failures.append(
                f"  {filename}: HASH MISMATCH (expected {expected_hash[:16]}..., "
                f"got {actual_hash[:16]}...)"
            )

    if failures:
        print(f"[FAIL] Hash Verification Failed for {engine_root.name}:")
        for f in failures:
            print(f)
        return False
    count = len([k for k in file_hashes if not k.endswith('.md')])
    print(f"[PASS] {engine_root.name}: {count} engine files match manifest")
    return True


def verify_hashes():
    """Verify file hashes for every engine in KNOWN_ENGINES. The runtime
    engine (ENGINE_VERSION) is strict; newer successor engines (EXPERIMENTAL)
    are strict only when a manifest file is present on disk — a missing
    manifest for a successor engine is tolerated so a half-built successor
    cannot block the FROZEN runtime's integrity check."""
    all_ok = True
    runtime_ver = ENGINE_VERSION
    for ver in KNOWN_ENGINES:
        engine_dir = PROJECT_ROOT / "engine_dev" / "universal_research_engine" / f"v{ver.replace('.', '_')}"
        manifest = engine_dir / "engine_manifest.json"
        if not engine_dir.exists():
            print(f"[SKIP] Engine directory absent: {engine_dir}")
            continue
        ok = _verify_manifest(engine_dir, manifest, strict=(ver == runtime_ver))
        if not ok:
            all_ok = False
    return all_ok
