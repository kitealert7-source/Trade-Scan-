...
*   **Actions Performed & Failure Conditions (Abort entire process if ANY fail):**
    ...
    2.  *Backtest Footprint Check:* For each `run_id` in `KEEP_RUNS`, verify that `TradeScan_State/backtests/<run_id>.json` exists.
...
| `TradeScan_State/backtests/` | `<run_id>.json` summary arrays | Filename stripping `.json` extension equals `run_id` | Must exist inside `KEEP_RUNS` | **Prune if missing** |
...
2.  All `run_ids` securely resting in `KEEP_RUNS` must possess required artifacts (`runs/<id>`, `backtests/<id>.json`).
...
*   For all elements in `KEEP_RUNS`, exact paths for `runs/<id>` and `backtests/<id>.json` MUST pre-exist or process aborts.
