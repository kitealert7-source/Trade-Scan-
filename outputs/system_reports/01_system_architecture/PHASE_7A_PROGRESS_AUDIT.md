...
| 2026-05-18 | Stage 5 replay STOPPED post kill-switch redesign deployment. 100% hard_fail rate on 4 strategies (AUDJPY/AUDUSD/BTCUSD/EURUSD) was unmasked by the redesign (previously hidden behind old HALT_EQUITY_DD fires). Finding: hard_fail is **expected behavior** in this replay setup — vault baselines are 2024 research trades, journal events are 2026 live signals; hashes cannot match by construction. NOT a validator wiring bug. | Stage 5's runtime-stability objective is already met (72h survival, no leaks, atomic writes, kill stickiness, new kill rule firing correctly on 3 genuinely catastrophic equity events incl. one negative-equity). Signal-validation aspect of Stage 5 was never meaningful — the test setup doesn't compare like-with-like. For NEW strategies coming online, validator works correctly in **live mode** (baseline + live signals from same strategy version). Pre-deployment vault smoke test (replay vault's own trade log through validator → expect 100% EXACT_MATCH) deferred to **time of actual strategy promotion** — added then, not built speculatively now. |
