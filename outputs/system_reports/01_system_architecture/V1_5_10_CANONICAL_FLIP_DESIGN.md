...
> **Status: EXECUTED 2026-06-17 — Phase B flip + promotion DONE** (commits `363c8179` flip, `cd2e229b` vault/FROZEN). Scope as run: §3 charge surgery + §5 promotion (vault v1_5_10 + EXPERIMENTAL→FROZEN). The LM/HF cherry-pick (P2/§6) and the corpus re-run (Phase 3/§7) were **explicitly OUT OF SCOPE** (operator: operational correctness, not viability). Positive proof: `tests/test_v1510_fast_path_charge.py`. A rerun↔namespace contract conflict surfaced during full-pipeline verification (basket reruns via `run_pipeline` mis-route through the auto-namespace-migration) — **chipped separately** (`task_f7f42597`), NOT part of this promotion. **UPDATE — Phase C also EXECUTED 2026-06-17:** single-asset `active_engine` flipped v1_5_8→v1_5_10 (commit `bb15c768`); the whole promotion merged to `main` via PR #3 (merge `d98a4770`). v1.5.10 is now the single canonical compute for BOTH paths; single-asset runs charge direction-aware spread and self-report the cost regime (`24c4b7e8`). The rerun↔namespace conflict was RESOLVED (`56dd03d7` + `c5e57512`; chip `task_f7f42597`). The "single-strategy out of scope / active_engine v1_5_8" framing above reflects the original Phase-B scope only. Below is the original design.
>
