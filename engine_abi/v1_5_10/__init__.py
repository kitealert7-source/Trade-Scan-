"""engine_abi.v1_5_10 — direction-aware ABI surface for basket_runner + TS_SignalValidator.
...
editing `governance/engine_abi_v1_5_10_manifest.yaml` first and re-hashing via
`python tools/abi_audit.py --rehash --abi-version v1_5_10`.
...
_MANIFEST_PATH = (
    ...
    / "governance"
    / "engine_abi_v1_5_10_manifest.yaml"
)
...
def _verify_against_manifest() -> None:
    if not _MANIFEST_PATH.is_file():
        raise RuntimeError(
            f"engine_abi.v1_5_10: manifest not found at {_MANIFEST_PATH}. "
            "Run `python tools/abi_audit.py --rehash --abi-version v1_5_10`."
        )
    ...
    if declared != list(__all__):
        raise RuntimeError(
            "engine_abi.v1_5_10 ABI manifest drift detected (FAIL-CLOSED).\n"
            f"  manifest exports: {declared}\n"
            f"  package __all__:  {list(__all__)}\n"
            "Fix: edit governance/engine_abi_v1_5_10_manifest.yaml and "
            "`python tools/abi_audit.py --rehash --abi-version v1_5_10`."
        )
