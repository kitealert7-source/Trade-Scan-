"""
46_STR_XAU_1H_CHOCH_S01_V3_P00 — Cross-Regime-Flip Probe Variant (XAU)

Variant of 46_STR_XAU_1H_CHOCH_S01_V2_P01 with a single added filter:
admit a trade ONLY when the regime at the signal bar differs from the
regime at the fill bar. All other parameters identical to V2_P01
(long-only, CHOCH_v2, ATR 1.5x SL / 3.0x TP, next_bar_open fills).

Filter-only use of regime_id_fill. Never consumed for signal construction.
Probe mode — inline in check_entry rather than a FilterStack subclass.
"""
...
class Strategy:
    ...
    # --- IDENTITY ---
    name = "46_STR_XAU_1H_CHOCH_S01_V3_P00"
    timeframe = "1h"
    ...
    def check_entry(self, ctx):
        ...
        # signal construction. Tail bar (NaN fill) is unreachable for
        # entries in this engine, so no NaN guard needed.
        try:
