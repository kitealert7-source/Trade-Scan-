...
def run_universal_engine(directive_path: str, strategy_name: str):
    ...
    decision, explanation = run_preflight(directive_path, engine_name, engine_version)
    print(f"[PREFLIGHT] Decision: {decision}")
    print(f"[PREFLIGHT] Reason: {explanation}")
    
    if decision == "HARD_STOP":
        print("[PREFLIGHT] HARD_STOP — Governance cannot be verified. Aborting.")
        sys.exit(2)
    elif decision == "BLOCK_EXECUTION":
        print("[PREFLIGHT] BLOCK_EXECUTION — Preflight check failed. Aborting.")
        sys.exit(1)
    # decision == "ALLOW_EXECUTION" → continue
