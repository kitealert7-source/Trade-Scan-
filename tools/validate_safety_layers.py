...
======================================================================
Runs six checks against a strategy's vault snapshot:
...
from execution_engine.strategy_guard import (
    StrategyGuard, GuardConfig, SignalResult,
...
# ---------------------------------------------------------------------------
# Test 6 — Two-tier validation + MismatchTracker
# ---------------------------------------------------------------------------
def test_two_tier_validation(vault_dir: pathlib.Path, profile: str) -> None:
    print(f"\n{SEP}")
    print("TEST 6 — Two-tier validation + MismatchTracker")
    print(SEP)
    ...
    try:
        ...
        print(f"  WARN: guard_bridge not importable — using inline tracker")
        # Minimal inline tracker for test
        class MismatchTracker:
            def __init__(self, **kwargs):
                self.total = self.exact = self.soft = self.hard = 0
            def record(self, r):
                self.total += 1
                if r.status == "EXACT_MATCH": self.exact += 1
                elif r.status == "SOFT_MATCH": self.soft += 1
                else: self.hard += 1

    guard = StrategyGuard.from_vault(vault_dir, profile)
    rows = _read_trade_log(vault_dir, profile)
    tracker = MismatchTracker(strategy_id="test")

    if not rows or not guard.baseline.signal_index:
        print(f"  SKIP — no signal index available")
        return

    # Run validate_signal against known trades (should be EXACT_MATCH)
    exact_count = 0
    for row in rows[:20]:  # test first 20 trades
    ...
    print(f"  Tested {tracker.total} signals from trade log")
    print(f"  EXACT_MATCH: {tracker.exact}")
    print(f"  SOFT_MATCH:  {tracker.soft}")
    print(f"  HARD_FAIL:   {tracker.hard}")

    # Test SOFT_MATCH: slightly alter price
    if rows:
        row = rows[0]
        price = float(row["entry_price"])
        soft_result = guard.validate_signal(
            symbol=row["symbol"],
            entry_timestamp=row["entry_timestamp"],
            direction=int(row["direction"]),
            entry_price=price * 1.00005,  # 0.005% off — within 0.1% tolerance
            risk_distance=float(row["risk_distance"]),
        )
        tracker.record(soft_result)
        print(f"  Slightly altered price ({price:.5f} -> {price * 1.00005:.5f}): {soft_result.status}")

    # Test HARD_FAIL: wildly wrong price
    if rows:
        row = rows[0]
        hard_result = guard.validate_signal(
            symbol=row["symbol"],
            entry_timestamp=row["entry_timestamp"],
            direction=int(row["direction"]),
            entry_price=float(row["entry_price"]) + 500.0,  # way off
            risk_distance=float(row["risk_distance"]),
        )
        tracker.record(hard_result)
        print(f"  Wildly wrong price: {hard_result.status}")
        if hard_result.status != "HARD_FAIL":
            print(f"  {FAIL} — expected HARD_FAIL for wildly wrong price")
            sys.exit(1)

    print(f"  Final counts: exact={tracker.exact} soft={tracker.soft} hard={tracker.hard} total={tracker.total}")
    print(f"  {PASS}")
...
def main():
    ...
    test_two_tier_validation(vault_dir, profile)
