...
# Self-generated artifacts excluded from the OPERATOR-FACING working-tree
# cleanliness metric — NOT from git tracking. These files are regenerated by
# this tool (SYSTEM_STATE.md, tools/TOOLS_INDEX.md), committed unconditionally
# by session-close, or are runtime data (data_root/), so a diff in them is never
# an actionable "dirty tree" signal. They stay tracked (preserving the committed
# historical record + the hand-authored Manual section); excluding them here only
# stops a false SESSION STATUS WARNING. The pre-push cleanliness check is separate
# (session-close 3.7).
SELF_GENERATED_STATUS_EXCLUSIONS = (
    "data_root/",
    "SYSTEM_STATE.md",
...
def extract_porcelain_path(line: str) -> str:
    """Return the path from one `git status --porcelain` (v1) line.

    Porcelain v1 lines are ``XY<space>PATH``, where ``XY`` is the two-character
    status code (e.g. ` M`, `M `, `MM`, `??`, `A `, `D `). Rename/copy entries are
    ``XY ORIG -> DEST``; we return DEST — the path as it exists in the working tree
    after the change, which is what the exclusion check cares about.

    The line MUST be the raw porcelain line (not pre-stripped): the status code
    occupies the first two columns, so stripping leading whitespace would shift the
    path offset. Returns "" for a malformed/short line.
    """
    if len(line) < 4:
        return ""
    path = line[3:]
    if " -> " in path:  # rename / copy — destination is the live working-tree path
        path = path.split(" -> ", 1)[1]
    return path.strip()


def is_status_noise(path: str) -> bool:
    """True if `path` is a self-generated artifact excluded from the operator-facing
    working-tree cleanliness metric (see SELF_GENERATED_STATUS_EXCLUSIONS). Operates
    on the parsed path, never on raw porcelain text."""
    return path.startswith(SELF_GENERATED_STATUS_EXCLUSIONS)
...
def collect_git() -> dict[str, Any]:
    ...
    try:
        ...
        if status.returncode == 0:
            # Parse -> filter -> present, as three separate concerns. The
            # `working_tree` string reports OPERATOR-actionable dirtiness, not raw
            # git-tree cleanliness: self-generated artifacts are parsed out so the
            # tool no longer flags files it regenerates itself. Untracked files are
            # deliberately still counted here (a separate semantic question — see
            # session-close 3.7 — out of scope for this metric).
            parsed_changes = [extract_porcelain_path(l)
                              for l in status.stdout.splitlines() if l.strip()]
            operator_changes = [p for p in parsed_changes
                                if p and not is_status_noise(p)]
            result["working_tree"] = (
                "clean" if not operator_changes
                else f"{len(operator_changes)} uncommitted"
            )
        else:
