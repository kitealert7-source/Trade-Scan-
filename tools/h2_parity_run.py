...
def build_parity_report(h2: pd.DataFrame, out_path: Path) -> None:
    ...
    # Research-stage baseline (per MEMORY + RESUME_USD_BASKET_NEXT_SESSION.md)
    ref_target = 7
    ref_blown = 2
    ref_none = 1
    ref_hit_rate = 0.7
    ref_mean_per_cycle = 511.0  # +$511 / cycle on $1k stake
    ...
    if n_total < 10:
        ...
        lines.append("✅ **Pipeline reproduces research-stage hit/blow distribution exactly.** "
                     "The H2_recycle@1 port is faithful to the basket_sim Variant G + harvest "
                     "+ compression-gate spec across all 10 windows.")
    else:
        ...
        lines.append("- spread/commission load on pipeline engine vs basket_sim simulator")
