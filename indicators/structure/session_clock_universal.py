"""
Session Clock — Universal Exchange-Session Boundaries
...
Same structural logic as session_clock.py. The only delta is SESSION_BOUNDARIES:
this module uses canonical UTC exchange-aligned windows that are designed to be
asset-portable rather than gold-liquidity-tuned.

Boundaries (UTC):
    asia:   00:00 - 08:00   (Tokyo cash + post-Tokyo morning, 8h)
    london: 08:00 - 14:00   (LSE open through pre-NYSE handoff, 6h)
    ny:     14:00 - 21:00   (NYSE cash open through close, 7h)
...
# --- Semantic Contract ---
SIGNAL_PRIMITIVE = "session_clock_universal"
PIVOT_SOURCE = "none"

# --- Universal Exchange-Session Boundaries (UTC hours) ---
SESSION_BOUNDARIES = {
    "asia":   (0, 8),
    "london": (8, 14),
    "ny":     (14, 21),
...
def session_clock(df: pd.DataFrame) -> pd.DataFrame:
    ...
    using canonical UTC exchange-aligned session boundaries.
    ...
    if (result["session_id"] == "none").all():
        raise RuntimeError(
            "session_clock_universal invariant violation: no real-session bars detected"
        )
