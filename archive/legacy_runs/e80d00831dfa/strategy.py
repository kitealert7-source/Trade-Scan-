"""
S07_V1_P02: uc_mode=balanced variant. min_bars_before_exit=3. All other logic from S07_P00.
"""
...
from indicators.price.ultimate_c_percent_variant import ultimate_c_percent
...
class Strategy:
    ...
    # --- IDENTITY ---
    name = "01_MR_FX_1H_ULTC_REGFILT_S07_V1_P02"
    timeframe = "1h"
    ...
    "indicators": [
        ...
        "indicators.price.ultimate_c_percent_variant",
    ...
    def __init__(self):
        ...
        entry_cfg = mr.get("entry", {})
        exit_cfg = mr.get("exit", {})
        stop_cfg = self.STRATEGY_SIGNATURE.get("execution_rules", {}).get("stop_loss", {})
        self.long_signal = entry_cfg.get("long", {}).get("ultimate_signal", -1)
        self.short_signal = entry_cfg.get("short", {}).get("ultimate_signal", 1)
        self.max_abs_trend = entry_cfg.get("long", {}).get("max_abs_trend_score", 1)
        self.uc_mode = str(entry_cfg.get("uc_mode", "balanced"))
    ...
    def prepare_indicators(self, df):
        ...
        uc_df = ultimate_c_percent(df, mode=self.uc_mode)
    ...
    def check_entry(self, ctx):
        ...
        if ult_sig == self.long_signal and direction != 1:
            self.session_low = row["close"] - risk_dist
            self.session_high = None
            return {"signal": 1}
        if ult_sig == self.short_signal and direction != -1:
            self.session_high = row["close"] + risk_dist
            self.session_low = None
            return {"signal": -1}
        return None
