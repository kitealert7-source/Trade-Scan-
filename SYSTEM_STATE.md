...
- **COINTREV v1.2 — build in-flight (resume from §11 of design doc):** SQLite loader + auto-join landed 2026-05-24 (commit `464e4a6`); strategy class + leg strategy + tests + dispatch + generator + pilot still to do (~4-6h focused). Architectural fork resolved in §11: use a new `CointTriggerLegStrategy` (matches H3_spread pattern). Actual first-pass scope is 263 directives @ lookback=252 (not 286). See [`DESIGN_DOC.md §11`](outputs/cointegration_screener_v1/v1_2_strategy_design/DESIGN_DOC.md).
