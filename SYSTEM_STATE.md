...
- **Phase 5b.3 carry-over — `lineage_pruner` blocked.** 322 alive `runs/<X>/directive_state.json` files have `attempts[latest].status == PORTFOLIO_COMPLETE` but reference child `run_ids` whose `runs/<rid>/` dirs are gone. `lineage_pruner --execute` bails on referential integrity with ~363 missing references. `repair_integrity --action drop` (Excel side) and `directive_reconciler --execute` (directives side) already ran 2026-05-20 — they handle different scopes. **Next session: build `tools/state_lifecycle/reconcile_portfolio_complete.py`** that surgically updates each `directive_state.json` to remove dead child run_ids (with `--dry-run` default + audit log). Alternative: extend `lineage_pruner._collect_portfolio_complete_runs()` to soft-check children (warn, not fail). Either unblocks 29 RUN_INCOMPLETE + 9 ABORTED still in `runs/`.
