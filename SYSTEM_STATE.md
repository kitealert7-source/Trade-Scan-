...
- **Phase 7a Stage 4 — PARTIAL coverage (open as a non-blocking concern).** Stage 1 proved *logic* determinism (port classification = reference, byte-identical 7,722 events). Stage 3 proved atomic-write consistency. Neither proves *full runtime* determinism — two consecutive `run_validator.py` invocations consuming the same shadow journal end-to-end producing byte-identical sequences of decision.json snapshots (same seq_no for the same events, same per-symbol aggregation state, same heartbeat extras). This is broader: it combines event replay + emitted-decision cadence + sequence persistence + multi-symbol state accumulation. Should be one explicit test in a future session: run validator twice with `--max-duration-s` fixed, snapshot the decision.json after each emit (via seq_no), assert sequences are identical. Not blocking Stage 2 — operator can pin this after the 1h run if needed.
