...
- [MPS_LINEAGE_DRIFT — 2026-05-26 weekend-maintenance] 17 orphan run_ids referenced in `Master_Portfolio_Sheet.xlsx` `constituent_run_ids` field have no physical artifact on disk. Distribution: 11 of 17 recoverable from past `TradeScan_State/quarantine/` dirs (20260521T014750Z_surgical_cleanup, 20260522_191517_cleanup); 6 unrecoverable. Affected parents: 5 Portfolios rows (22_CONT_FX_30M_RSIAVG_TRENDFILT_S02_V1_P03 + 4× PF_* composites) + 8 Single-Asset Composites rows (XAU/FX/ETH variants). Impact: `tools/state_lifecycle/lineage_pruner.py --execute` blocked by integrity check (fails-closed, correct). Pipeline runs unaffected — Stage 3/4 add new rows cleanly. Resolution NOT actioned this session because: (1) `repair_integrity.py` has two safety bugs — violates append-only invariant + would destroy MPS sub-sheets (SAC + Baskets, see `tmp/PIPELINE_STATE_DRIFT_2026-05-26.md`); (2) tagging via quarantine_status requires schema change (column doesn't exist on Portfolios/SAC sheets) + safe-writer coordination + downstream-consumer audit. Earliest revisit: defer to first session that's pursuing workbook-lifecycle remediation OR until pruner --execute is actually needed.
