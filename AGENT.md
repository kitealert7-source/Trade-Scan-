...
11. **Protected Infrastructure** — `tools/`, `engines/`, `engine_dev/`, `governance/`, `.claude/skills/` require implementation plan + explicit human approval before modification. *Advisory enforcement (2026-07-01):* the `commit-msg` hook `tools/lint_protected_infra_approval.py` (installed via `tools/hooks/install.sh`) blocks a commit staging protected-dir files unless the message carries a `Protected-Infra-Approved: <reason>` trailer (auto-regen data — `tools_manifest.json`, namespace registries — exempt). Audit trail: `git log --grep Protected-Infra-Approved`. Deliberate-acknowledgment gate, not cryptographic.
...
26. **Sequential by Default; Opt-In Parallelism** — `run_pipeline.py --all` runs directives sequentially by default (`--max-parallel 1`, permanent first-class mode for debug / reproducibility / emergency recovery). The legacy 15-second cooldown was **REMOVED 2026-05-27**; Stage 3 + Stage 4 FileLocks (`Strategy_Master_Filter.xlsx.lock`, `Master_Portfolio_Sheet.xlsx.lock`) provide deterministic exclusivity around shared ledger writes. `--max-parallel N` opts into cross-directive `ProcessPoolExecutor` parallelism — evidence-driven; the default never silently flips. (Was "15-second cooldown / batch prohibited" — stale since 2026-05-27; aligned with CLAUDE.md #7 on 2026-07-01.)
...
32. **Viewing-Layer / Ledger Separation (Excel is a projection, SQL is the ledger)** — `ledger.db` (`master_filter`, `basket_sheet`, `cointegration_sheet`, …) is the authoritative ledger; the Excel workbooks (`Master_Portfolio_Sheet.xlsx`, `Filtered_Strategies_Passed.xlsx`) are a **regenerated VIEWING LAYER ONLY** — `ledger_db.py --export-*` rebuilds them wholesale from the DB through code-defined view projections (`tools/portfolio/*_view.py`). Therefore: **(a)** NEVER add a column or datum to an xlsx by hand — the next regenerate silently wipes it (the 2026-06-15 `spans`/`fragment_count` column was lost exactly this way). The displayed column set + order live in the view-projection code, which is the SSOT for the view, locked by a column-budget test. **(b)** The view-projection modules (`portfolio/cointegration_view.py`, `portfolio/trade_candidates_view.py`) are in the Critical Guard Set (`generate_guard_manifest.py` `GUARD_FILES`), so a silent column drop/rename freezes pipeline execution until a human re-blesses the manifest. **(c)** An xlsx-only operator annotation that must survive regeneration goes through the documented audit-columns preserve mechanism (`ledger_db.py`), never ad-hoc.
