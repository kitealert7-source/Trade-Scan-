"""engine_abi.v1_5_11 acceptance + bootstrap-consumer test.

This module is cited as the sole `consumed_by` for every entry in
`governance/engine_abi_v1_5_11_manifest.yaml` until the canonical flip re-points
basket_runner (+ TS_SignalValidator) to v1_5_11. Replacing entries here with the
...
Identity guarantee: every symbol exposed by engine_abi.v1_5_11 must be the exact
object exported by its source module (`is`-identity, since the ABI is a re-export
package). Mirror of test_engine_abi_v1_5_9 for the v1.5.11 successor surface.
...
from engine_abi import v1_5_11 as abi

# Direct source-module imports — what the ABI re-exports must be `is`-identical to.
from engine_dev.universal_research_engine.v1_5_11 import evaluate_bar as _eb
from engine_dev.universal_research_engine.v1_5_11 import execution_loop as _el
...
_MANIFEST_PATH = _REPO_ROOT / "governance" / "engine_abi_v1_5_11_manifest.yaml"
...
def test_manifest_loads():
    ...
    assert manifest["abi_version"] == "v1_5_11"
...
def test_package_all_matches_manifest():
    ...
    assert list(abi.__all__) == declared, (
        f"engine_abi.v1_5_11 __all__ does not match manifest.\n"
...
def test_engine_version_is_1_5_11():
    assert abi.ENGINE_VERSION == "1.5.11"
...
def test_manifest_hash_is_stamped():
    ...
    assert re.fullmatch(r"[0-9a-f]{64}", h), (
        ...
        f"`python tools/abi_audit.py --rehash --abi-version v1_5_11` "
        f"(currently {h!r})"
    )
