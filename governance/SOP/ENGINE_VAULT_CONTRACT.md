...
5. **Durable docs name the NEW canonical** — **this contract's §7 note**, the canonical-flip
   design doc, and memory (the *manually-maintained* records; a stale canonical reference in
   these is a closure FAIL). `SYSTEM_STATE.md` is NOT in this list — it is auto-regenerated by
   `/session-close` (`system_introspection.py`) and reads the active engine from the registry
   automatically; do not hand-edit it to record a promotion (the record lives in §13).
