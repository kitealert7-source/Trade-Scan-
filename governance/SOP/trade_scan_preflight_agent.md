...
DIRECTIVE
  ↓
Step 0 — Read AGENT.md Failure Playbook
  ↓
Step 1 — SOP Ingestion (read all governing SOPs)
  ↓
Step 1.5 — Mode Classification (CLONE_MODE or GENESIS_MODE)
  ↓
Step 1.75 — Directive Canonicalization Gate

- python tools/canonicalizer.py <DIRECTIVE_ID>.txt
- Structural drift → fix with --execute before proceeding
  ↓
...
Stage-0.75 Dry-Run Validation
  - Executes strategy on a minimal data window
  - Confirms trade generation (non-zero trade count)
  - NO_TRADES result → BLOCK (directive shelved)
  ↓
...
Stage-0.75 MUST NOT execute unless Stage-0.5 passes.
Stage-1 MUST NOT execute unless Stage-0.75 passes.
...
- Stage-0.75 Dry-Run Validation may run only after `PREFLIGHT_COMPLETE_SEMANTICALLY_VALID`
- Stage-1 may run only after `DRY_RUN_VALID`
