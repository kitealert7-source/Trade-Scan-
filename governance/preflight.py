...
def run_preflight(
    ...
    # §1A Provisioner Safeguard: backup existing strategy.py before overwrite.
    ...
    if _strategy_name:
        ...
        if _existing_strategy.exists():
            import shutil
            _bak_path = _existing_strategy.with_suffix(".py.bak")
            shutil.copy2(str(_existing_strategy), str(_bak_path))
            print(f"[PREFLIGHT] Provisioner safeguard: backed up strategy.py -> strategy.py.bak")
    ...
    # Authority: trade_scan_preflight_agent.md (Stage-0.75)
    # Quick validation that strategy produces trades on a minimal data window.
    ...
    if _strategy_folder:
        ...
        if _strategy_file.exists():
            print(f"[PREFLIGHT] Stage-0.75: Dry-run validation for {_strategy_folder}...")
            try:
                import importlib.util
                _spec = importlib.util.spec_from_file_location("strategy_module", str(_strategy_file))
                _mod = importlib.util.module_from_spec(_spec)
                _spec.loader.exec_module(_mod)
                _cls = getattr(_mod, "Strategy", None)
                if _cls is None:
                    return ("BLOCK_EXECUTION", f"Stage-0.75: Strategy class not found in {_strategy_folder}/strategy.py", None)
                print(f"[PREFLIGHT] Stage-0.75: Strategy class loaded successfully.")
            except Exception as e:
                return ("BLOCK_EXECUTION", f"Stage-0.75 Dry-Run FAILED: Cannot import strategy: {e}", None)
