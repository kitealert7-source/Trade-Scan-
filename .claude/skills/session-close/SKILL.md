...
# Stage the regenerated sources, then commit (paths explicit so none are missed):
...
The commands ARE the Phase 3 step blocks above, executed in fixed `3.x` order
(3.1 → 3.A → 3.B → … → 3.13 → Phase 4). No separate copy-paste list is
maintained — it duplicated the longhand and drifted from it (friction log
2026-05-17: one fix had to land in two places). Run each `### 3.x` section in
sequence; conditional letter-steps either run or print `[SKIP] reason: <signal>`
